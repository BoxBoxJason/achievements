...
suite('Controllers Test Suite', () => {
    ...
    test('AchievementController.getAchievableAchievementsByCriterias should return achievements', async () => {
        ...
        const result = await AchievementController.getAchievableAchievementsByCriterias([constants.criteria.FILES_CREATED]);
    ...
    test('AchievementController.getAchievements should handle filters', async () => {
        ...
        const result = await AchievementController.getAchievements({ limit: 1, offset: 0 });
        assert.strictEqual(result.achievements.length, 1);

        // Test sort
        const result2 = await AchievementController.getAchievements({ sortCriteria: 'title', sortDirection: 'ASC' });
        ...
        // Test trimming
        const result3 = await AchievementController.getAchievements({ title: '  ' });
        // Should ignore empty title
        assert.ok(result3.achievements.length > 0);

        // Test category trimming
        const result4 = await AchievementController.getAchievements({ category: '  ' });
        assert.ok(result4.achievements.length > 0);

        // Test group trimming
        const result5 = await AchievementController.getAchievements({ group: '  ' });
        assert.ok(result5.achievements.length > 0);

        // Test criterias trimming
        const result6 = await AchievementController.getAchievements({ criterias: ['  ', ''] });
        assert.ok(result6.achievements.length > 0);

        // Test labels trimming
        const result7 = await AchievementController.getAchievements({ labels: ['  ', ''] });
    ...
    test('TimeSpentController.updateTimeSpentFromSessions should update progressions', async () => {
        ...
        const progressions = await Progression.getProgressions({ name: constants.criteria.DAILY_TIME_SPENT });
    ...
    test('TimeSpentController.getTimeSpent should return counters', async () => {
        // Setup progressions
        const p1 = new Progression({ name: constants.criteria.DAILY_TIME_SPENT, value: 100, type: 'number' });
        const p2 = new Progression({ name: constants.criteria.TWO_WEEKS_TIME_SPENT, value: 200, type: 'number' });
