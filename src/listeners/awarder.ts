 ...
 *
 * @param {{ id: number, title : string , exp: number , achievedAt : string}} achievement - The achievement to award
 *
 * @returns {Promise<void>} - A promise that resolves when the achievement is awarded
 */
export async function awardAchievement(achievement : { id: number, title : string , exp: number , achievedAt : string}): Promise<void> {
  const message = `Achievement unlocked: ${achievement.title} (${achievement.exp} exp)`;
  if (config.notificationsEnabled()) {
    vscode.window.showInformationMessage(`🏆 ${message}`, 'Browse Achievements')
      .then((selection) => {
